# /raptor-fuzz:fuzz - RAPTOR Binary Fuzzer
...
3. **Run RAPTOR fuzzing**: Execute the fuzzing command:
   ```bash
   python3 $RAPTOR_ROOT/raptor.py fuzz --binary <path> --duration <seconds>
   ```
...
```bash
python3 $RAPTOR_ROOT/raptor.py fuzz --binary /path/to/binary --duration 6600
```

Quick fuzz (10 minutes):
```bash
python3 $RAPTOR_ROOT/raptor.py fuzz --binary /path/to/binary --duration 600 --max-crashes 5
```

With custom corpus:
```bash
python3 $RAPTOR_ROOT/raptor.py fuzz --binary /path/to/binary --corpus /path/to/seeds --duration 3600
```
...
- Python >= 3.9
- AFL++ (`afl-fuzz`, `afl-clang-fast`)

If AFL++ is not installed, display:
```
ERROR: AFL++ is not installed or not in PATH.

Install AFL++ from: https://github.com/AFLplusplus/AFLplusplus

For macOS: brew install afl-fuzz
For Linux: apt install afl++ or build from source
