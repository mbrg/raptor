# /raptor-oss-forensics:oss-forensics - OSS GitHub Forensic Investigation
...
1. **Read the orchestration skill:**
   Load the orchestration skill from this plugin's skills directory.
...
If credentials are missing, display:
```
...
5. Set: export GOOGLE_APPLICATION_CREDENTIALS=/path/to/credentials.json

Note: BigQuery offers 1 TB/month free queries.
...
```
/raptor-oss-forensics:oss-forensics "Investigate lkmanka58's activity on aws/aws-toolkit-vscode"

/raptor-oss-forensics:oss-forensics "Validate claims in this vendor report: https://example.com/report"

/raptor-oss-forensics:oss-forensics "What happened with the stability tag on aws/aws-toolkit-vscode on July 13, 2025?"

/raptor-oss-forensics:oss-forensics "Investigate the July 13 incident on aws/aws-toolkit-vscode" --max-followups 5
```
...
- Spawn followup investigations if oss-hypothesis-former-agent identifies loose ends
